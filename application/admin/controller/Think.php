...
class Think extends Admin{
    ...
    public function lists($model = null, $p = 0){
        ...
        return view($model['template_list']);
    }
    ...
    public function edit($model = null, $id = 0){
        ...
        if(Request()->isPost()){
			...
			return view($model['template_edit']?$model['template_edit']:'');
        }
    }
    ...
    public function add($model = null){
        ...
        if(Request()->isPost()){
            ...
            return view($model['template_add']?$model['template_add']:'');
        }
    }
    ...
    protected function checkAttr($Model,$model_id){
        ...
        foreach($fields as $key=>$attr){
            if($attr['is_must']){// 必填字段
				$rule[$attr['name']][]  =  'require';
				$msg[$attr['name'].'.require'] = $attr['title'].'必须!';
            }
            // 自动验证规则
            if(!empty($attr['validate_rule'])) {
				...
				$rule[$attr['name']][] = $attr['validate_rule'];
				$msg[$attr['name'].'.'.$attr['validate_rule']] = $attr['error_info']?$attr['error_info']:$attr['title'].'验证错误';
            }
            // 自动完成规则
